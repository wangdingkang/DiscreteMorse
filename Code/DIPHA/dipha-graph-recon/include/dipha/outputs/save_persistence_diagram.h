...
int compute_original_index(const int64_t complex_idx, const int64_t nx, const int64_t ny, const int64_t nz)
{
        int cnx = 2 * nx - 1;
        int cny = 2 * ny - 1;
        int cnz = 2 * nz - 1;

        //std::cout << "complex id: " << complex_idx;

        int z = complex_idx / (2 * cnx * cny);
        //std::cout << " z: " << z;

        int xy_plane = complex_idx - z * (2 * cnx * cny);
        //std::cout << " xy only: " << xy_plane;
        int y = xy_plane / (2 * cnx);
        //std::cout << " y: " << y;
        int x_line = xy_plane - y * (2 * cnx);
        //std::cout << " x only: " << x_line;
        int x = x_line / 2;
        //std::cout << " x: " << x;
        int og_idx = x + nx * y + nx * ny * z;
  ...
  namespace outputs
  {
	...
	*/
	    ...
	    if (birth_dim == 0)
	    {
		    ...
		    std::pair<int, std::pair<double, double> > edge = std::make_pair(compute_original_index(test_bdr_a[0], nx, ny, nz), std::make_pair((double) compute_original_index(test_bdr_a[1], nx, ny, nz), -(death_value - birth_value) -1));
	    ...
	    else if (birth_dim == 1)
	    {
		    ...
		    std::pair<int, std::pair<double, double> > edge = std::make_pair(compute_original_index(test_bdr_a[0], nx, ny, nz), std::make_pair((double) compute_original_index(test_bdr_a[1], nx, ny, nz), death_value - birth_value));
                    //std::pair<int64_t, int64_t > edge_verts_pair = std::make_pair(test_bdr_a[0], test_bdr_a[1]);
                    //std::pair<int64_t, double > edge_type_per_pair = std::make_pair(1, death_value - birth_value);
                    //local_edges.push_back(std::make_pair(edge_verts_pair, edge_type_per_pair));
		    local_edges.push_back(edge);
