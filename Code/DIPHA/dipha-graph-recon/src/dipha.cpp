...
void parse_command_line(int argc, char** argv, bool& benchmark, bool& dualize, int64_t& upper_dim, std::string& input_filename, 
                        std::string& output_filename, std::string& edge_filename, int64_t& nx, int64_t& ny, int64_t& nz)
{
  ...
  input_filename = argv[argc - 6];
  output_filename = argv[argc - 5];
  edge_filename = argv[argc - 4];
  nx = std::atoi(argv[argc - 3]);
  ny = std::atoi(argv[argc - 2]);
  nz = std::atoi(argv[argc - 1]);

  //std::cout << "Check: " << nx << std::endl;

  for (int idx = 1; idx < argc - 6; idx++)
  {
...
void compute(const std::string& input_filename, bool dualize, int64_t upper_dim, const std::string& output_filename, const std::string& edge_filename, int64_t nx, int64_t ny, int64_t nz)
{
  Complex complex;
  ...
  DIPHA_MACROS_BENCHMARK(dipha::outputs::save_persistence_diagram(output_filename, edge_filename, nx, ny, nz, complex, filtration_to_cell_map, reduced_columns, 
                                                                  dualize, upper_dim); );
}
...
int main(int argc, char** argv)
{
  ...
  parse_command_line(argc, argv, benchmark, dualize, upper_dim, input_filename, output_filename, edge_filename, nx, ny, nz);
  ...
  case dipha::file_types::IMAGE_DATA:
    compute< dipha::inputs::weighted_cubical_complex >(input_filename, dualize, upper_dim, output_filename, edge_filename, nx, ny, nz);
    break;
  default:
