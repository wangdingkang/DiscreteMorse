...
def read_ve(swc_file):
    ...
    #     for line in file:
    #         edge = tuple([int(x)-1 for x in line.strip().split()[:2]])
    #         edges.append(edge)
...
def get_all_segs(nodes, edges, z_range):
    ...
    for z in range(z_range):
        ...
        for e in range(len(edges)):
            ...
            u = nodes[edge[0]]
            v = nodes[edge[1]]
            if in_between(z-2, u[2], v[2]):
                seg = segment(u, v, z-2)
                density = (u[3] + v[3]) / 2
                # density = get_density(seg, cloud_list[z])
                seg_all[z].append((seg, 1, e))  # seg = (x1, y1, x2, y2), density, id(e))
                max_density = max(max_density, density)
...
def make_png(seg_all, z_range, dir_path, max_density, ind_arr, l=3000, w=2250, max_width=1):
    ...
    for z in range(z_range):
        ...
        for seg in seg_all[z]:
            density = int(255 * seg[1] / max_density)
            # print(seg, density)
...
if __name__ == '__main__':
    ...
    # make pngs
    image_path_seg = os.path.join(dir_name, output_dir)
    make_png(seg_all, z_range, image_path_seg, 1, ind_array, l=length, w=width)
